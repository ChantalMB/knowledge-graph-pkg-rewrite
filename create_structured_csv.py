...
def main():
    ...
    for file in pickles:
        ...
        flag = True
        for part in file_name_list[1:]:
            file_name += '_'
            file_name += part
            print(file_name)

        print(file_name)
        # skip spacy parsing mistakes --> may be eliminating important things tho...
        df = pd.read_csv(curr_dir +"/data/output/kg/"+file_name+".txt-out.csv", error_bad_lines=False)
